...
// Workspace Types - Replaces Family Types
// Supports: individuals, couples, families, teams, and organizations

export type HabitNexModeId = 'personalPulse' | 'partnerBoost' | 'householdHarmony' | 'familyOrbit';
export type WorkspaceTabId = 'overview' | 'members' | 'habits' | 'challenges' | 'rewards' | 'analytics' | 'settings';
export type WorkspaceType = 'personal' | 'couple' | 'family' | 'team' | 'organization';

export interface Workspace {
  id: string;
  name: string;
  type: WorkspaceType;         // New: Defines workspace context
  createdBy: string;           // User ID of workspace creator
  createdAt: Timestamp;
  updatedAt: Timestamp;
  inviteCode: string;          // 6-digit code for joining workspace
  isActive: boolean;
  isPersonal?: boolean;        // True for auto-created personal workspaces
...
export interface WorkspaceSettings {
  ...
  theme: 'light' | 'dark';
  avatarStyle: 'fun-emoji' | 'bottts' | 'personas' | 'adventurer';  // Consistent avatar style for whole workspace
...
export interface WorkspaceMember {
  ...
  role: WorkspaceRole;         // Updated: Generic role system
...
export interface WorkspaceHabitCompletion {
  id: string;
  workspaceId: string;
  habitId: string;
  memberId: string;
  ...
  // Workspace features
  encouragements: Encouragement[];  // From other workspace members
...
export interface Reward {
  ...
  // Admin controls
  requiresApproval: boolean;    // Admin must approve redemption
  budgetCost?: number;          // Real money cost (for purchases)

  // Metadata
  createdBy: string;            // Member ID (usually admin/owner)
  createdAt: Timestamp;
  updatedAt: Timestamp;
...
export interface RewardRedemption {
  ...
  // Approval workflow
  approvedBy?: string;          // Admin/owner member ID
...
export interface WorkspaceMoodEntry {
  id: string;
  workspaceId: string;
  memberId: string;
  date: string;                // YYYY-MM-DD
  ...
  // Workspace features
  sharedWithWorkspace: boolean;    // Visible to other workspace members
  adminNotification: boolean;      // Alert admins if mood is very low
}
...
export interface CreateWorkspaceHabitRequest {
  workspaceId: string;
  habit: Omit<WorkspaceHabit, 'id' | 'workspaceId' | 'createdAt' | 'updatedAt' | 'isActive' | 'isArchived'>;
}

export interface CreateRewardRequest {
  workspaceId: string;
  reward: Omit<Reward, 'id' | 'workspaceId' | 'createdAt' | 'updatedAt' | 'totalRedemptions'>;
}

// Dashboard Types
export interface WorkspaceDashboardData {
  workspace: Workspace;
  todayHabits: WorkspaceHabit[];
  completions: WorkspaceHabitCompletion[];
  memberStats: Record<string, MemberStats>;
  activeRewards: Reward[];
  pendingRedemptions: RewardRedemption[];
  activeChallenges: WorkspaceChallenge[];
}

// Touch Screen Types
export interface TouchScreenSettings {
  displayMode: 'workspace' | 'member' | 'mixed';
...
export type WorkspaceRole = 'owner' | 'admin' | 'member' | 'viewer' | 'parent' | 'child' | 'teen' | 'adult'; // Includes legacy family roles for compatibility
...
// Utility Types for Database Migration
...
// Helper type for workspace type validation
export function isValidWorkspaceType(type: string): type is WorkspaceType {
  return ['personal', 'couple', 'family', 'team', 'organization'].includes(type);
}
...
export function canApproveRedemptions(role: WorkspaceRole): boolean {
  return ['owner', 'admin', 'parent'].includes(role);
}
