 ...
 */
export async function recoverOrphanedWorkspaces(userId: string, userEmail: string): Promise<boolean> {
  try {
    console.log('🔧 Starting family recovery for:', userEmail);
    
    // First, check if user already has families (no recovery needed)
    const familiesSnapshot = await getDocs(collection(db, 'workspaces'));
    ...
    for (const familyDoc of familiesSnapshot.docs) {
      ...
      const membersSnapshot = await getDocs(collection(db, 'workspaces', familyDoc.id, 'members'));
      ...
      for (const memberDoc of membersSnapshot.docs) {
        ...
        if (
          ...
          if (workspaceData.createdBy === memberDoc.id || memberData.role === 'parent') {
            console.log(`🎯 Found potential orphaned family: ${workspaceData.name}`);
            console.log(`   Old member ID: ${memberDoc.id}, New user ID: ${userId}`);
            
            // Create a new member document with the current user ID
            const newMemberRef = doc(db, 'workspaces', familyDoc.id, 'members', userId);
            ...
            console.log(`✅ Recovered family "${workspaceData.name}" for user`);
            familiesRecovered++;
  ...
  try {
    ...
    if (!querySnapshot.empty) {
      ...
      if (!existingMember.exists()) {
        ...
        console.log(`✅ Recovered family "${workspaceData.name}" using invite code`);
        return true;
      }
    }
